...
if (!isset($_SESSION['logged_in']) || !$_SESSION['logged_in']) {
	header('Location: ../wartaberita1.0/ganti_password.php');
	die;
}

require_once('Connections/koneksi.php');
$user = mysqli_fetch_array($koneksi->query("SELECT * FROM `anggota` WHERE `no_induk`= '".$_SESSION['current_id']."'"));
...
$updatepassword= $koneksi->query("UPDATE `anggota` SET `password` = '$passwordbaru' WHERE `no_induk`= '".$_SESSION['current_id']."'");
